...
一个可直接预览与部署的全栈网页应用。输入公开站点 URL，即可获得带评分、证据、优先级、修复建议和验收方式的完整报告，并下载 JSON、Markdown 或智能体提示词包。当前版本为 `1.3.1`。
...
- **结果评分**：以 100 分为满分，只对已验证问题扣分，同时显示证据覆盖率、扣分依据和不计分的待补证据。
...
- 单次最多巡检 300 个同源页面；同一应用实例同时只运行一项巡检。
